		...
		<div class="w960  clf">
								...
								<ul id="menu">
									...
									<ul id="child1" style="DISPLAY: none">
										<c:if test="${!empty  friend}">
											<c:forEach items="${friend}" var="friend" varStatus="s">
												<li>
												<a href="<%=path%>/showinfor?friendid=${friend.friendid}" target="iframepage"><img
									...
									<ul id="child2" style="DISPLAY: none">
										<c:if test="${!empty  lahei}">
											<c:forEach items="${lahei}" var="deletefriend"
												varStatus="s">
												<li>
											 <a href="<%=path%>/showinfor?friendid=${deletefriend.friendid}" target="iframepage">
												<img src="headpic/${deletefriend.user.headpic }" width="25"
									...
									<ul id="child3" style="DISPLAY: none">
										<c:if test="${!empty  qingqiu}">
											<c:forEach items="${qingqiu}" var="waitfriend"
												varStatus="s">
												<li>
												 <a href="<%=path%>/showinfor?friendid=${waitfriend.friendid} target="iframepage">
												<img src="headpic/${waitfriend.user.headpic }" width="25" height="25" />${waitfriend.user.username}</a>
												</li>
											</c:forEach>
										</c:if>
									</ul>
			...
			<div class="m_main">
				<iframe frameborder=0 width=840 id="iframepage" name="iframepage"
					onLoad="iFrameHeight()" 
					src="<%=request.getContextPath()%>/askforfriend"></iframe>
			</div>
		</div>
	</div>
